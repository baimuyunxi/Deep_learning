...
# 加载训练好的模型
model = load_model(
    "model/rinse/test/event_type.h5", custom_objects={"Attention": Attention}
)

with open("model/rinse/test/event_type.json", "r", encoding="utf-8") as f:
...
def predict_single_text(text):
    ...
    indices = sorted(
        [i for i in range(len(predicted)) if predicted[i] > 0.6],
...
df.to_csv("model/rinse/test/pred_result.csv", index=False)
