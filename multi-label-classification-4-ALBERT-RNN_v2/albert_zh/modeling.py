    ...
    def __init__(
        ...
        with tf.compat.v1.variable_scope(scope, default_name="bert"):
            ...
            with tf.compat.v1.variable_scope("encoder"):
                ...
                if (
                    ln_type == "postln" or ln_type is None
                ):  # currently, base or large of albert used post-LN structure
                    print(
                    ...
                    print(
                        "new structure of transformer.use: prelln_transformer_model,which use pre-LN"
                    )
...
def prelln_transformer_model(
    ...
    for layer_idx in range(num_hidden_layers):
        ...
        with tf.compat.v1.variable_scope(
            idx_scope["layer"], reuse=tf.compat.v1.AUTO_REUSE
        ):
            layer_input = prev_output

            with tf.compat.v1.variable_scope(
                idx_scope["attention"], reuse=tf.compat.v1.AUTO_REUSE
            ):
                attention_heads = []

                with tf.compat.v1.variable_scope(
                    "output", reuse=tf.compat.v1.AUTO_REUSE
                ):
            ...
            with tf.compat.v1.variable_scope(
                idx_scope["output"], reuse=tf.compat.v1.AUTO_REUSE
            ):
                attention_output_pre = layer_norm(attention_output)

            # The activation is only applied to the "intermediate" hidden layer.
            with tf.compat.v1.variable_scope(
                idx_scope["intermediate"], reuse=tf.compat.v1.AUTO_REUSE
            ):
                intermediate_output = tf.compat.v1.layers.dense(
