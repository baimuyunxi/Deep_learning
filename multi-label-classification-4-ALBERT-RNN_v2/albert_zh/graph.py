...
def import_tf(device_id=-1, verbose=False):
    ...
    tf.compat.v1.logging.set_verbosity(
        tf.compat.v1.logging.DEBUG if verbose else tf.compat.v1.logging.ERROR
    )
    return tf
...
def set_logger(context, verbose=False):
    ...
    formatter = logging.Formatter(
        "%(levelname)-.1s:"
        + context
        + ":[%(filename).5s:%(funcName).3s:%(lineno)3d]:%(message)s",
        datefmt="%m-%d %H:%M:%S",
    )
    ...
    try:
        ...
        with tf.compat.v1.variable_scope("pooling"):
            ...
            if pooling_strategy == PoolingStrategy.REDUCE_MEAN:
                ...
                pooling_strategy == PoolingStrategy.FIRST_TOKEN
                or pooling_strategy == PoolingStrategy.CLS_TOKEN
            ):
                pooled = tf.squeeze(encoder_layer[:, 0:1, :], axis=1)
            elif (
                pooling_strategy == PoolingStrategy.LAST_TOKEN
                or pooling_strategy == PoolingStrategy.SEP_TOKEN
            ):
        ...
        # print("pooling strategy-------------------------->", pooled)
        pooled = tf.identity(pooled, "final_encodes")
        # print("pooling finale------------------------>", pooled)
