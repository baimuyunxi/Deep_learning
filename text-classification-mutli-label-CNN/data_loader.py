...
def build_vocab(train_dir, vocab_dir, vocab_size=5000):
    ...
    open(os.path.join(vocab_dir, "vocab.txt"), mode="w", encoding="utf-8").write(
        "\n".join(words) + "\n"
    )
    open(os.path.join(vocab_dir, "label.txt"), mode="w", encoding="utf-8").write(
        "\n".join(all_label) + "\n"
    )


def read_vocab(vocab_dir):
    with open(os.path.join(vocab_dir, "vocab.txt"), encoding="utf-8") as fp:
...
def read_category(vocab_dir):
    with open(os.path.join(vocab_dir, "label.txt"), encoding="utf-8") as fp:
